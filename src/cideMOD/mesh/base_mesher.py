...
class Cylinder(BaseMesher):
    def build_mesh(self):
        ...
        #Translation, purely put in by hand. The mesh creates a box which is partially 
        #empty. This translation just centers the cylinder in the first quadrant
        t=Point(1.5,(1.128379167095513-0.5641895835477563),-(0.36952429155263994 + 0.5641895835477563))
        rot.translate(self.mesh, t)
        #Cylinder now along x-axis rather than z-axis in order to be consistent with cide

        #Debug: print extremal positions along each axis
        #print(max(self.mesh.coordinates()[:,2]))
        #print(min(self.mesh.coordinates()[:,2]))
        #print(max(self.mesh.coordinates()[:,1]))
        #print(min(self.mesh.coordinates()[:,1]))
        #print(max(self.mesh.coordinates()[:,0]))
        #print(min(self.mesh.coordinates()[:,0]))
        ...
        if self.structure[-1] in ('c','pcc') or not 'c' in self.structure:
            #negativetab = subdomain_generator.set_boundary(0)
            #positivetab = subdomain_generator.set_boundary(n)
            negativetab=CompiledSubDomain("(x[0]-0<tol && x[0]-0>0)|| (0-x[0]<tol && 0-x[0]>0)",tol=tol)
            positivetab=CompiledSubDomain("(x[0]-3<tol && x[0]-3>0)|| (3-x[0]<tol && 3-x[0]>0)",tol=tol)
        else:
            negativetab = subdomain_generator.set_boundary(n)
            positivetab = subdomain_generator.set_boundary(0)
        negativetab.mark(boundaries, self.field_data['negativePlug'])
        positivetab.mark(boundaries, self.field_data['positivePlug'])
        XDMFFile("Box_checks/cylinder_verify_boundaries.xdmf").write(boundaries)
        tabs = subdomain_generator.set_boundaries(0, n)
        ...
        anode=CompiledSubDomain("(x[0]>=-tol && x[0]<=1+tol) && (((x[1]-0.5641895835477563)*(x[1]-0.5641895835477563) + (x[2]-0.5641895835477563)*(x[2]-0.5641895835477563))<(0.5641895835477563*0.5641895835477563) + tol)",tol=tol)
        separator=CompiledSubDomain("(x[0]>=1-tol && x[0]<=2+tol)  && (((x[1]-0.5641895835477563)*(x[1]-0.5641895835477563) + (x[2]-0.5641895835477563)*(x[2]-0.5641895835477563))<(0.5641895835477563*0.5641895835477563) + tol)",tol=tol)
        cathode=CompiledSubDomain("(x[0]>=2-tol && x[0]<=3+tol)  && (((x[1]-0.5641895835477563)*(x[1]-0.5641895835477563) + (x[2]-0.5641895835477563)*(x[2]-0.5641895835477563))<(0.5641895835477563*0.5641895835477563) + tol)",tol=tol)
        #to be complete
        negative_cc = subdomain_generator.set_domain(negative_cc_list)
        positive_cc = subdomain_generator.set_domain(positive_cc_list)

        #electrodes = subdomain_generator.electrodes(self.structure)
        sub_electrode= MeshFunction("size_t", self.mesh, self.mesh.topology().dim(), 0)
        electrodes=CompiledSubDomain("((x[0]>=-tol && x[0]<=1+tol) || (x[0]>=2-tol && x[0]<=3+tol))  && (((x[1]-0.5641895835477563)*(x[1]-0.5641895835477563) + (x[2]-0.5641895835477563)*(x[2]-0.5641895835477563))<(0.5641895835477563*0.5641895835477563) + tol)",tol=tol)
        electrodes.mark(sub_electrode,10)
        XDMFFile("Box_checks/cylinder_verify_electrode.xdmf").write(sub_electrode)
        #electrolyte = subdomain_generator.electrolyte(self.structure)
        sub_electrolyte= MeshFunction("size_t", self.mesh, self.mesh.topology().dim(), 0)
        electrolyte=CompiledSubDomain("(x[0]>=-tol && x[0]<=3+tol) && (((x[1]-0.5641895835477563)*(x[1]-0.5641895835477563) + (x[2]-0.5641895835477563)*(x[2]-0.5641895835477563))<(0.5641895835477563*0.5641895835477563) + tol)",tol=tol)
        electrolyte.mark(sub_electrolyte,10)
        XDMFFile("Box_checks/cylinder_verify_electrolyte.xdmf").write(sub_electrolyte)
        #solid_conductor = subdomain_generator.solid_conductor(self.structure)
        solid_conductor=CompiledSubDomain("((x[0]>=-tol && x[0]<=1+tol) || (x[0]>=2-tol && x[0]<=3+tol)) && (((x[1]-0.5641895835477563)*(x[1]-0.5641895835477563) + (x[2]-0.5641895835477563)*(x[2]-0.5641895835477563))<(0.5641895835477563*0.5641895835477563) + tol)",tol=tol)
        #Aka, never, no current collectprs
        current_colectors = CompiledSubDomain("false",tol=tol)

        #CCs will not be marked, but anode, separator, cathode should be correct
        anode.mark(subdomains,self.field_data['anode'])
        separator.mark(subdomains,self.field_data['separator'])
        cathode.mark(subdomains,self.field_data['cathode'])
        XDMFFile("Box_checks/cylinder_verify_subdomains_pre.xdmf").write(subdomains)
        ...
        XDMFFile("Box_checks/cylinder_verify_subdomains.xdmf").write(self.subdomains)
        ...
        anode_CC_interface = CompiledSubDomain("false",tol=tol)
        cathode_CC_interface = CompiledSubDomain("false",tol=tol)
        ...
        #xdmf file to verify subdomains 
        XDMFFile("Box_checks/cylinder_verify_interfaces.xdmf").write(self.interfaces)
